...
parfor unitind=1:length(dounits);
    ...
    current_waveforms = load([penultwavedir 'waveforms_i' num2str(1) '_cl' num2str(unit) '.mat'])   
    
    bestchan=bestchannel{unit};
    channeltipoffset=(round(z(bestchan))+tipelectrode)/1000;   %height offset between tip and best channel, in mm. this is an estimate of the depth of the unit relative to probe tip.
    ...
    currentplotchannels=[currentplotchannels; bestchan];  %concatenation fix issue, use ';'
    
    if length(current_waveforms.waveforms{bestchan})==0
        continue
    end
    ...
    wavesinchani=current_waveforms.waveforms{channel};
        ...
        axis([min(x)-100 max(x)+100 min(z)-100 max(z)+100])
    ...
    wavesinbestchan=current_waveforms.waveforms{bestchan};
